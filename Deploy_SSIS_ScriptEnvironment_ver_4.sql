...
-- To copy environment settings from one ETL SSIS project to another:

-- 'source' = SSISDB that you want to copy environment config info from
-- 'target' = SSISDB that you want to copy environment config info to

-- Run this script on the 'source' server to create a stored procedure that will copy environment configuration data from SSISDB
-- Execute the stored procedure on the 'source' server and supply values for SSISDB folder and environment node name
-- Look in the messages tab when the sproc completes, this is where the 'target' script is generated
-- Copy all of the contents in the messages tab this is the script to run on the target to copy the env settings but omit the last line
-- Scrub the generated 'target' script by replacing the 'source' server name with the 'target' server name
-- Run the generated script on the 'target' server
-- Refresh the SQL object browser and expand the SSISDB and project, you should see the new environment configuration 
--
-- You may have to script each ETL job to 'drop and create' and replace the /ENVREFERENCE command line parameter with the 
--     actual environment reference value. Run this query to get the environment reference value:

--     SELECT  reference_id
--     FROM  SSISDB.[catalog].environment_references er
--     JOIN SSISDB.[catalog].projects p ON p.project_id = er.project_id
--     WHERE  er.environment_name = @envName
--     AND  p.name              = @EtlJobName;

-- You will also have to add each newly created environment to the matching SSIS project configuration then pull in the environment
--     values from the newly created environment in the SSIS project configuration.

CREATE PROCEDURE [dbo].[pDeploy_SSIS_ScriptEnvironment_ver_4]
          @folder sysname
        , @env sysname
...
AS
        ...
        */
        ...
        SELECT [cmd] = @tab + 'RAISERROR(''Creating variable: ' + ev.name + ' ...'', 10, 1) WITH NOWAIT;' + @cr
                                        ...
                                        + @tab + @tab + ', @sensitive=''' + CONVERT(nvarchar(256), ev.sensitive) + '''' + @cr
                                        + @tab + @tab + ', @description=N''' + ev.[description] + '''' + @cr
                                        + @tab + @tab + ', @environment_name=@env' + @cr
                                        + @tab + @tab + ', @folder_name=@folder' + @cr


                                        --+ @tab + @tab + ', @value=@env_value' + @cr
										+ 
										(
										select case 
											when ev.type='String'
												then + @tab + @tab + ', @value=N' + '''' + convert(nvarchar(256), ev.value) + '''' + @cr
											else + @tab + @tab + ', @value=' + convert(nvarchar(256), ev.value) + @cr
										end
										)
